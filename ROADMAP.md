...
| ~~`ref-20` finengy-navy-notched (lot 14)~~ | **soldée le 21/08/2026** | 1 pattern versé — `card-15-notched-float-statement`. Première source INTERNE du corpus (un écran conçu ici, pas un visuel tiers relevé). Le lot établit deux lois de rendu qu'aucune mesure de composition ne voyait : un filtre s'applique AVANT le clip-path (une ombre posée sur l'élément chanfreiné est découpée avec lui et disparaît en silence), et `drop-shadow()` n'accepte pas de rayon d'étalement (une quatrième longueur invalide le filtre entier, toujours en silence). Au passage : la césure suppose un `lang` déclaré, donc un fragment autonome le porte lui-même | — |
